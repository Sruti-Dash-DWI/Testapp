...
//     <div className="flex flex-col text-white p-4 md:p-6 h-full bg-gradient-to-l from-[#6366f1] via-[#a5b4fc] to-[#e0e7ff]">
//       <AddTaskModal
...
const Dropdown = ({ button, children, align = 'right' }) => {
  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      <button
        onClick={() => setIsOpen((prev) => !prev)}
        className="p-2 rounded-lg bg-white/20 backdrop-blur-sm border border-white/30 text-gray-800 hover:bg-white/30 focus:outline-none flex items-center gap-2"
      >
        {button}
      </button>
      {isOpen && (
...
const TesterBoard = () => {
  ...
  return (
    <TesterDashboardLayout>
    <div className="flex flex-col text-gray-800 p-4 md:p-6 h-full bg-gradient-to-l from-[#6366f1] via-[#a5b4fc] to-[#e0e7ff]">
      <AddTaskModal
        show={isModalOpen || !!editingTask}
        onHide={() => {
          handleCloseModal();
      ...
      <header className="flex-shrink-0 flex justify-between items-center mb-6">
        <div className="flex items-center gap-4">
          <button className="text-gray-800 p-2 rounded-lg hover:bg-black/20">
            <MenuIcon />
          </button>
          <Dropdown
            button={
        ...
        <div className="hidden lg:flex flex-1 justify-center px-8 ">
          <input type="text" placeholder="Search tasks, projects..." value={searchTerm} onChange={handleSearchChange} className="w-full max-w-lg p-2.5 border border-white/30 rounded-full focus:ring-2 focus:ring-blue-500 focus:outline-none shadow-sm bg-white/20 backdrop-blur-sm text-gray-800 placeholder:text-gray-600" />
        </div>
        <div className="flex items-center gap-2 md:gap-5">
          <button className="hidden md:flex items-center gap-2 p-2 rounded-lg text-gray-600 hover:bg-black/20">
            <ShareIcon /> <span >Share</span>
      ...
      <div className="flex-shrink-0 flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
        <div className="flex items-center gap-3 self-start w-full md:w-auto">
          ...
          <button onClick={() => { setIsModalOpen(true); setNewTaskColumnId(null) }} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-white/20 backdrop-blur-sm border border-white/30 text-gray-800 hover:bg-white/30">
            <span className="text-lg font-bold text-gray-800">+</span> <span className="text-gray-800">Add Task</span>
          </button>
        </div>
        <div className="flex items-center gap-3 self-end w-full md:w-auto justify-end">
          <div className="relative">
            {isLocalSearchOpen ? (<input type="text" placeholder="Search..." value={searchTerm} onChange={handleSearchChange} className="w-48 p-2 rounded-lg border border-black/40 bg-white/10 backdrop-blur-sm text-gray-700 placeholder:text-gray-700 focus:ring-2 focus:ring-gray-400 focus:outline-none" />
            ) : (<button onClick={() => setIsLocalSearchOpen(true)} className="p-2.5 rounded-lg bg-black/20 backdrop-blur-sm border border-white/30 text-gray-800 hover:bg-black/30">
              <SearchIcon />
            </button>
            )}
          </div>
          <button className="p-2 rounded-lg bg-black/20 backdrop-blur-sm border border-white/30 text-gray-800 hover:bg-black/30">
            <KebabMenuIcon />
          </button>
        </div>
      </div>
